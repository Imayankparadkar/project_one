import React, { useState } from "react";
import { Plus, X, Link as LinkIcon, FileText } from "lucide-react";
import { CampaignFormData } from "../types";

interface Step4RequirementsProps {
    formData: CampaignFormData;
    setFormData: React.Dispatch<React.SetStateAction<CampaignFormData>>;
}

export default function Step4Requirements({ formData, setFormData }: Step4RequirementsProps) {
    // State for inputs
    const [newRequirement, setNewRequirement] = useState("");

    // State for adding links
    const [isAddingLink, setIsAddingLink] = useState(false);
    const [newLink, setNewLink] = useState("");

    // State for adding FAQs
    const [isAddingFAQ, setIsAddingFAQ] = useState(false);
    const [newFAQ, setNewFAQ] = useState({ question: "", answer: "" });

    // Styles
    const inputClass = "w-full rounded-lg border border-(--color-border-input) bg-(--color-bg-input) bg-yellow-radial px-3 py-2 text-sm leading-5 tracking-[0.14px] text-(--color-text-primary) placeholder-(--color-text-muted) outline-none transition-all focus:border-(--color-border-input) focus:ring-1 focus:ring-(--color-border-input)";
    const addButtonClass = "px-4 py-2 rounded-lg bg-(--color-bg-input-foc) border border-(--color-border-input-foc) text-(--color-text-primary) text-sm font-medium hover:bg-(--color-bg-input) transition-colors shrink-0";
    const cardClass = "p-4 rounded-xl border border-(--color-border-card) bg-(--color-bg-card)";

    // --- HELPER FUNCTIONS ---

    const addRequirement = () => {
        if (!newRequirement.trim()) return;
        setFormData(prev => ({ ...prev, requirements: [...(prev.requirements || []), newRequirement] }));
        setNewRequirement("");
    };

    const addLink = () => {
        if (!newLink.trim()) return;
        setFormData(prev => ({ ...prev, contentLinks: [...(prev.contentLinks || []), newLink] }));
        setNewLink("");
        setIsAddingLink(false);
    };

    const addFAQ = () => {
        if (!newFAQ.question.trim() || !newFAQ.answer.trim()) return;
        setFormData(prev => ({ ...prev, faqs: [...(prev.faqs || []), newFAQ] }));
        setNewFAQ({ question: "", answer: "" });
        setIsAddingFAQ(false);
    };

    const AIButton = () => (
        <div className="group/ai relative">
            <div className="p-[1.5px] rounded-lg bg-ai-gradient">
                <button 
                    type="button" 
                    className="text-(--color-text-muted) hover:text-(--color-text-secondary) bg-[var(--color-bg-card)] p-1.5 rounded-[7px] transition-all cursor-pointer active:scale-95 flex items-center justify-center shrink-0"
                >
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="animate-grow-in">
                        <path d="M3.3877 9.29102C3.64259 8.65445 4.54384 8.65455 4.79883 9.29102L5.14551 10.1572C5.27246 10.4746 5.5245 10.7264 5.8418 10.8535L6.70898 11.2002C7.34583 11.455 7.34565 12.3564 6.70898 12.6113L5.8418 12.958C5.52446 13.085 5.2726 13.337 5.14551 13.6543L4.79883 14.5215C4.54403 15.1585 3.64248 15.1585 3.3877 14.5215L3.04102 13.6543C2.91386 13.3371 2.66208 13.0849 2.34473 12.958L1.47852 12.6113C0.84189 12.3564 0.841704 11.455 1.47852 11.2002L2.34473 10.8535C2.66233 10.7265 2.91398 10.4748 3.04102 10.1572L3.3877 9.29102ZM4.09277 11.3193C3.928 11.5435 3.73096 11.7405 3.50684 11.9053C3.73104 12.07 3.92792 12.2681 4.09277 12.4922C4.25754 12.268 4.45458 12.0701 4.67871 11.9053C4.45485 11.7405 4.25736 11.5434 4.09277 11.3193ZM8.77832 1.66016C9.06321 0.779286 10.3342 0.779308 10.6191 1.66016L10.6436 1.75195L11.1445 3.91894C11.2521 4.38448 11.6155 4.74899 12.0811 4.85644L14.249 5.35644C15.2509 5.58813 15.251 7.01553 14.249 7.24707L12.0811 7.74707C11.6155 7.85455 11.2521 8.21806 11.1445 8.68359L10.6436 10.8516C10.4119 11.8535 8.98554 11.8535 8.75391 10.8516L8.25293 8.68359C8.14534 8.21807 7.78197 7.85456 7.31641 7.74707L5.14844 7.24707C4.14646 7.0155 4.14659 5.58815 5.14844 5.35644L7.31641 4.85644C7.78201 4.74899 8.14532 4.38448 8.25293 3.91894L8.75391 1.75195L8.77832 1.66016ZM9.69824 4.31641C9.44971 5.29199 8.68863 6.05261 7.71289 6.30078C8.6887 6.54886 9.44965 7.31054 9.69824 8.28613C9.94669 7.31074 10.7082 6.54914 11.6836 6.30078C10.7082 6.05233 9.94663 5.29178 9.69824 4.31641Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd"></path>
                    </svg>
                </button>
            </div>
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover/ai:opacity-100 transition-opacity pointer-events-none z-50">
                <div className="bg-zinc-800 text-white text-[9px] font-medium px-2 py-0.5 rounded shadow-xl whitespace-nowrap border border-white/10 uppercase tracking-tight">
                    Generate using AI
                    <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-zinc-800" />
                </div>
            </div>
        </div>
    );


    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">

            {/* 1. CAMPAIGN REQUIREMENTS */}
            <div className="space-y-3">
                <div className="flex items-center justify-between">
                    <label className="text-sm font-semibold text-(--color-text-primary)">Campaign Requirements <span className="text-[#f9a84b]">*</span></label>
                    <AIButton />
                </div>

                {/* List of added requirements */}
                {formData.requirements?.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                        {formData.requirements.map((req, idx) => (
                            <div key={idx} className="bg-(--color-bg-tertiary) text-(--color-text-primary) text-xs pl-2 pr-1 py-1 rounded-md border border-white/10 flex items-center gap-2 group">
                                <span>{req}</span>
                                <button
                                    onClick={() => setFormData(prev => ({ ...prev, requirements: prev.requirements.filter((_, i) => i !== idx) }))}
                                    className="text-(--color-text-primary) cursor-pointer p-0.5 rounded-sm hover:bg-white/10 transition-colors"
                                >
                                    <X className="w-3 h-3" />
                                </button>
                            </div>
                        ))}
                    </div>
                )}

                <div className="flex gap-2">
                    <input
                        type="text"
                        value={newRequirement}
                        onChange={(e) => setNewRequirement(e.target.value)}
                        placeholder="Enter Requirement"
                        className={inputClass}
                        onKeyDown={(e) => e.key === "Enter" && addRequirement()}
                    />
                    <button onClick={addRequirement} className={addButtonClass}>Add</button>
                </div>
            </div>

            {/* 2. CONTENT LINKS */}
            <div className="space-y-3">
                <div className="flex items-center justify-between">
                    <label className="text-sm font-semibold text-(--text-color-primary)">Assets</label>
                </div>

                {/* List of added links */}
                {formData.contentLinks?.length > 0 && (
                    <div className="space-y-2 mb-3">
                        {formData.contentLinks.map((link, idx) => (
                            <div key={idx} className={cardClass + " flex items-center justify-between py-3"}>
                                <div className="flex items-center gap-3 overflow-hidden">
                                    <div className="p-2 rounded-lg bg-(--color-bg-card) text-(--color-text-primary) shrink-0">
                                        <LinkIcon className="w-4 h-4" />
                                    </div>
                                    <a href={link} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-400 hover:underline truncate">{link}</a>
                                </div>
                                <button
                                    onClick={() => setFormData(prev => ({ ...prev, contentLinks: prev.contentLinks.filter((_, i) => i !== idx) }))}
                                    className="text-(--color-text-secondary) p-1.5 rounded-lg hover:bg-white/10 transition-colors shrink-0"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ))}
                    </div>
                )}

                {isAddingLink ? (
                    <div className={cardClass + " space-y-3 animate-in fade-in zoom-in-95 duration-200"}>
                        <div className="space-y-1">
                            <label className="text-xs font-medium text-(--color-text-secondary)">Link URL</label>
                            <input
                                type="url"
                                value={newLink}
                                onChange={(e) => setNewLink(e.target.value)}
                                placeholder="Paste URL"
                                className={inputClass}
                                onKeyDown={(e) => e.key === "Enter" && addLink()}
                                autoFocus
                            />
                        </div>
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setIsAddingLink(false)} className="px-3 py-1.5 text-xs font-medium text-(--color-text-muted) hover:text-(--color-text-secondary) transition-colors">Cancel</button>
                            <button onClick={addLink} className={addButtonClass + " py-1.5 text-xs"}>Add Link</button>
                        </div>
                    </div>
                ) : (
                    <button
                        onClick={() => setIsAddingLink(true)}
                        className="w-full py-3 rounded-xl border border-dashed border-(--color-border-input) bg-(--color-bg-input) text-(--color-text-secondary) text-sm font-medium hover:cursor-pointer transition-all flex items-center justify-center gap-2"
                    >
                        <Plus className="w-4 h-4" /> Add content link
                    </button>
                )}
            </div>

            {/* 3. FAQs */}
            <div className="space-y-3">
                <label className="text-sm font-semibold text-(--color-text-primary)">FAQs</label>

                {/* List of added FAQs */}
                {formData.faqs?.length > 0 && (
                    <div className="space-y-2 mb-3">
                        {formData.faqs.map((faq, idx) => (
                            <div key={idx} className={cardClass + " flex items-start justify-between py-3 group"}>
                                <div className="flex gap-3 overflow-hidden">
                                    <div className="p-2 mt-1 rounded-lg shrink-0">
                                        <FileText className="w-4 h-4" />
                                    </div>
                                    <div className="space-y-0.5">
                                        <p className="text-sm font-medium text-(--color-text-primary) truncate">{faq.question}</p>
                                        <p className="text-xs text-(--color-text-secondary) truncate line-clamp-1">{faq.answer}</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => setFormData(prev => ({ ...prev, faqs: prev.faqs.filter((_, i) => i !== idx) }))}
                                    className="text-(--color-text-secondary) p-2 mt-1 rounded-lg hover:bg-(--color-bg-card) transition-colors shrink-0 ml-2"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ))}
                    </div>
                )}

                {isAddingFAQ ? (
                    <div className={cardClass + " space-y-3 animate-in fade-in zoom-in-95 duration-200"}>
                        <div className="space-y-1">
                            <input
                                type="text"
                                value={newFAQ.question}
                                onChange={(e) => setNewFAQ({ ...newFAQ, question: e.target.value })}
                                placeholder="Question"
                                className={inputClass}
                                autoFocus
                            />
                        </div>
                        <div className="space-y-1">
                            <textarea
                                value={newFAQ.answer}
                                onChange={(e) => setNewFAQ({ ...newFAQ, answer: e.target.value })}
                                placeholder="Answer"
                                className={inputClass + " min-h-[80px] resize-none"}
                            />
                        </div>
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setIsAddingFAQ(false)} className="px-3 py-1.5 text-xs font-medium text-(--color-text-muted) hover:text-(--color-text-secondary) transition-colors">Cancel</button>
                            <button onClick={addFAQ} className={addButtonClass + " py-1.5 text-xs"}>Add FAQ</button>
                        </div>
                    </div>
                ) : (
                    <button
                        onClick={() => setIsAddingFAQ(true)}
                        className="w-full py-3 rounded-xl border border-dashed border-(--color-border-input) bg-(--color-bg-input) text-(--color-text-secondary) text-sm font-medium hover:cursor-pointer transition-all flex items-center justify-center gap-2"
                    >
                        <Plus className="w-4 h-4" /> Add FAQ
                    </button>
                )}
            </div>
        </div>
    );
}